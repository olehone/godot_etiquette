...
@export var eat_particles_scene: PackedScene  # Assign your particles here
...
func _attach_to_utensil(area: Area3D) -> void:
	...
	if utensil and utensil is RigidBody3D:
		...
		if attach_point:
			...
			global_transform.origin = utensil.global_transform.origin + attach_offset
...
func _trigger_eat_effect():
	var particles = get_tree().get_root().get_node("XROrigin3D/XRCamera3D/GPUParticles3D")
	if particles and particles is GPUParticles3D:
		particles.emitting = true
	else:
		print("Effects node not found or not a particles node.")
